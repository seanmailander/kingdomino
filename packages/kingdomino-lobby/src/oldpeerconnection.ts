// TODO: This file is an old reference implementation that predates the current
// package architecture. It uses redux-saga and imports that no longer exist in
// this package (redux-saga, game.slice, gamelogic/fetch). It belongs neither
// in kingdomino-lobby (which should be framework-agnostic) nor in its current
// form (Redux/saga-coupled). It should either be deleted or fully rewritten as
// part of the new PeerSession/WebRtcTransport design in peer.session.ts.
// See architecture-report §9.5 and §9.6 for the correct placement of lobby logic.
// @ts-nocheck — suppressing type errors on this legacy file pending removal.
// This file is hte prior implementation of the peer connection logic, using the PeerJS library. It is left here for reference, but is not currently used in the application. The current implementation uses a custom WebSocket-based protocol instead.
// See how it interacts with the server
// The use of redux-saga is not deprecated
